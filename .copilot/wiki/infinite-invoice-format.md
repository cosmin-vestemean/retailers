...
**Status 2026-08-27: RESOLVED AND VERIFIED LIVE on both retailers.** Invoices generated by
`S1/JS/AJS/InfiniteInvoice.js` and sent through the app are now **accepted by Infinite EDInet for
both Auchan and Dedeman** (confirmed by the beneficiary in the portal, the only authoritative
source).

The rejections were **never about the XML**. The builder produces a correct native
`<Invoice Version="1.0.1">` document (verified element-by-element against genuinely accepted
invoices: 372/372 and 393/393 matches). Every invoice was rejected as "Invalid file structure"
because `edi-invoices.class.js` wrapped the XML in an S/MIME envelope before upload, so the file
landing on Infinite's FTP began with `Content-Type: multipart/signed` instead of `<?xml`. Signing
was activated earlier the same day and has been removed. Full proof, including a byte-level A/B of
accepted vs rejected files pulled off Infinite's own FTP: see **"ROOT CAUSE, PROVEN"** below —
read that section first; several earlier sections record hypotheses that were subsequently
disproven and are kept only for their reasoning lessons.

**Final confirmation**: after removing the signing step, all 6 Auchan invoices
(`FAEX1-PF-40689/90/93/94/95/96`) plus Dedeman `FAEXD-PF-40697` were re-uploaded as plain XML —
each exactly ~1 868 B smaller than its signed predecessor (the PKCS#7 block) — and accepted.
...
### ROOT CAUSE, PROVEN (2026-08-27): we were uploading an S/MIME envelope, not XML

**Every hypothesis below about field values was wrong. The file we uploaded was never an XML
document at all.** Proven by reading both files back off Infinite's own FTP with the project's own
`FtpTransport`, from `/invoice/archive/`, same directory, same day:

| File | Size | First bytes | Outcome |
|---|---|---|---|
| `FAEXD-PF-40697...` (button, 18:12) | 16 225 B | `<?xml version="1.0" encoding="iso-8859-2"?>` | **accepted** |
| `FAEX1-PF-40689...` (ours, 19:30) | 18 072 B | `Content-Type: multipart/signed; protocol="application/pkcs7-signature"...` | **rejected** |

The ~2 KB difference is the PKCS#7 signature block. Infinite's parser read the first line, found no
XML document, and reported the generic **"Invalid file structure"** — which is why *no* amount of
field fixing changed anything, and why the failure rate was exactly 100%.

**How it got there**: commit `49dd17b0` (2026-08-27, same session) activated `signSmime()` for
`provider.code === 'infinite'`. Before that the function existed but was never called. The
justification was a code comment — *"Infinite requires a detached S/MIME signature over the raw
XML"* — an assumption never verified against the channel. The rejection window starts exactly at
that commit.

**Fix**: the signing call was removed from `src/services/edi-invoices/edi-invoices.class.js` (plus
the orphaned import). `src/edi/sign-smime.js` is deliberately **kept** — see below.

**Is signing tied to SFTP?** No. S/MIME signs the *document*; SFTP encrypts the *channel* — they
are orthogonal. `documentatie/EDInet_Connector/EDInet Connector.md` shows the invoice relation as
`DIRECTORY -> FTP` to `ftp.infinite.pl:21`, plain passive FTP (the same channel we use,
`CONNTYPE=4`), and offers **three** signature options: `Not sign` / `Key storage` /
`Card Signature`. Signing is a per-relation setting agreed with Infinite's helpdesk, not a
protocol requirement. No SFTP endpoint at Infinite is documented anywhere; this repo's SFTP
transport is for DocProcess.

**So the accurate statement is: this account's relation is configured `Not sign`.** If Infinite
ever switches it to `Key storage`, signing must come back — hence `sign-smime.js` stays. When that
day comes, align the digest first: the connector uses **SHA1withRSA**, our implementation used
SHA-256, and our envelope also lacked a `MIME-Version: 1.0` header. **Do not re-enable signing on
the strength of a code comment again — require written confirmation from Infinite that the
relation was changed.**

**Method lesson (the expensive one)**: both locally available signals — the FTP
`MessageAcknowledgement` in `/logs/ok/` and the file appearing in `/invoice/archive/` — looked
healthy throughout, and the generated XML was eventually verified correct down to 372/372
elements. The check that settled it in minutes was **reading the first bytes of the artefact
actually uploaded**. When a document is rejected as malformed, verify the bytes on the wire
*before* auditing the content that produced them.

### What the XSD settled (downloaded 2026-08-27 from the schemaLocation URL)

`http://www.infinite.pl/pub/doc/fmt/xml/invoice/1.0/invoice.xsd` (redirects to `pub.infinite.pl`)
plus its 11 `universal_types/*.xsd` includes. Authoritative types:

| Element | Declared type | Consequence |
|---|---|---|
| `Tel` | `xsd:string`, nillable, optional | `0723.319.834` was always valid — `PHONE1` was never a defect |
| `BuyerOrderDate` | **`xsd:dateTime`** | the `T00:00:00` form is correct; date-only is the type error |
| `DeliveryDate`, `DeliveryDocumentDate` | `xsd:dateTime` | ours *and* the accepted files send date-only — tolerated |
| `BuyerSellerID` | `xsd:string`, nillable, `minOccurs=0` | an empty value is **not** a schema violation |
| `PacketContentQuantity` | `Dec133` | empty is invalid, but accepted Dedeman files do it 10× — tolerated |
| `TaxCategoryCoded` | enumeration | both `S` and `3D` are permitted |

**Critically: Infinite does NOT enforce strict XSD validation.** Validating the *accepted* files
gives 2 errors (Auchan) and 33 errors (Dedeman). Our own output produced the identical 33-error
set for Dedeman. So the XSD is useful as a *relative* comparison tool, never as an acceptance
oracle.
...
*(superseded — the actual cause was the S/MIME envelope, see "ROOT CAUSE, PROVEN" above. All three
below were real defects worth fixing, but none of them caused the rejections.)*
1. **`<BuyerSellerID>` empty** (both retailers) — real gap, `RECADV.js` never copied `CCCSELLERID`.
   Fixed. The XSD later showed an empty value is schema-legal, so this was never fatal.
2. **`Tel` from `PHONE1`** — cosmetic deviation from the proven scripts. Fixed to `PHONE2`.
   The XSD shows `Tel` is a plain optional string, so this was never fatal either.
