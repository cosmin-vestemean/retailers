...
- DocProcess shared-inbox routing fix 2026-06-10: `CCCSFTPXML=7314` is actually Kaufland (`TRDR=12349`, GLN `4049728610005`, order `3200159108`) but was inserted under Carrefour (`TRDR=11322`) because all DocProcess configs share the same `ORDERS_` inbox and scanner processed configs in TRDR order. `src/edi/scanner.js` now resolves DocProcess order retailer after download by parsing XML GLN and looking up `TRDBRANCH.CCCS1DXGLN` among active DocProcess retailers before inserting/backup metadata. Regression test added for Carrefour-first/Kaufland-second shared inbox routing. Validation: `npm test` passes (37 tests). Existing bad row `7314` still needs data correction if it should be processed: move to `TRDR_RETAILER=12349`, reset `XMLSTATUS='NEW'`, clear `XMLERROR`.
