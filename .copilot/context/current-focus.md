...
- DocProcess shared-inbox routing fix 2026-06-10: `CCCSFTPXML=7314` is actually Kaufland (`TRDR=12349`, GLN `4049728610005`, order `3200159108`) but was inserted under Carrefour (`TRDR=11322`) because all DocProcess configs share the same `ORDERS_` inbox and scanner processed configs in TRDR order. `src/edi/scanner.js` now resolves DocProcess order retailer after download by parsing XML GLN and looking up `TRDBRANCH.CCCS1DXGLN` among active DocProcess retailers before inserting/backup metadata. Follow-up decision: no ambiguity allowed. If DocProcess XML parsing, GLN extraction, active-candidate lookup, or GLN match fails, scanner fails closed with `DocProcess routing error...`; it does not insert under the current config retailer. Routing failures are moved to DO retry metadata with `retailer=0`; `do-retry` skips those objects until there is an explicitly resolved retailer, so they cannot be reinserted under a guessed retailer. Regression tests cover Carrefour-first/Kaufland-second routing and no-match fail-closed behavior. Validation: `npm test` passes (39 tests). Existing bad row `7314` still needs data correction if it should be processed: move to `TRDR_RETAILER=12349`, reset `XMLSTATUS='NEW'`, clear `XMLERROR`.
